#!/usr/bin/env python
"""Evaluate user×item score files via AUC/Logloss/MSE/RMSE."""
...
def compute_metrics(y_true: np.ndarray, y_score: np.ndarray) -> tuple[float, float, float, float]:
    # logloss は 0/1 端点で発散するため、微小値でクリップして数値安定化。
    eps = 1e-6
    ...
    try:
        ...
        ll = float("nan")
    err = clipped - y_true
    # MSE/RMSE は確率予測の「距離」を見る補助指標（閾値非依存）。
    mse = float(np.mean(np.square(err)))
    rmse = float(np.sqrt(mse))
    return auc, ll, mse, rmse
...
def main() -> None:
    ...
    rows = []
    if args.by == "overall":
        auc, ll, mse, rmse = compute_metrics(df[args.label_col].to_numpy(), df[args.pred_col].to_numpy())
        rows.append({"key": "overall", "auc": auc, "logloss": ll, "mse": mse, "rmse": rmse, "n": int(df.shape[0])})
    else:
        ...
        for key, group in df.groupby(group_col):
            auc, ll, mse, rmse = compute_metrics(group[args.label_col].to_numpy(), group[args.pred_col].to_numpy())
            rows.append({"key": key, "auc": auc, "logloss": ll, "mse": mse, "rmse": rmse, "n": int(group.shape[0])})
