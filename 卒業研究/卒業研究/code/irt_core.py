...
機能:
    - 3PL の正答確率計算や logit 変換、BKT→θ 変換ユーティリティ
    - 項目パラメータ CSV・θ CSV を標準カラムへ正規化して読み込む
    - offline_predict_scores などから簡単に IRT 計算を呼び出せるよう支援
"""
...
def bkt_prob_to_theta(p: float) -> float:
    ...
    現実の推定 theta ではなく、BKT 習得確率を IRT スケールに写像した近似値。
    """
    return logit(p)
