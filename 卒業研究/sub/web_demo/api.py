...
def _select_openai_model(mode: str, override: Optional[str] = None) -> str:
    if override:
        return override
    if mode == "accurate":
        return OPENAI_MODEL_ACCURATE
...
def _call_openai_llm(
    ...
    if not prompt.get("candidates"):
        return None
    model = _select_openai_model(mode, model_override)
    system = (
...
def load_items() -> pd.DataFrame:
    ...
    # domain 列が無い場合は L2 を domain として使う（L2 は保持）
...
def load_logs() -> pd.DataFrame:
    ...
    if "p_L_after" not in df.columns:
        df["p_L_after"] = None
    if "tier" in df.columns and "L2" in df.columns and "L2_tier" not in df.columns:
        df["L2_tier"] = df["L2"].astype(str) + ":t" + df["tier"].astype(str)
    return df
...
def load_bkt_params() -> tuple[Dict[str, Dict[str, float]], Dict[str, Dict[str, float]]]:
    if not bkt_core:
        return {}, {}
    ...
    if "skill_field" in df.columns:
        l2_df = df[df["skill_field"] == "L2"]
        tier_df = df[df["skill_field"] == "L2_tier"]
    l2_params = {}
    tier_params = {}
    for _, row in l2_df.iterrows():
        l2_params[str(row["skill_name"])] = {
            "L0": float(row["L0"]),
            "T": float(row["T"]),
            "S": float(row["S"]),
            "G": float(row["G"]),
    ...
    for _, row in tier_df.iterrows():
        tier_params[str(row["skill_name"])] = {
            "L0": float(row["L0"]),
            "T": float(row["T"]),
            "S": float(row["S"]),
            "G": float(row["G"]),
        }
    return l2_params, tier_params


items_df = load_items()
scores_df = load_scores()
logs_df = load_logs()
bkt_params_l2, bkt_params_tier = load_bkt_params()
state_cache_l2: Dict[str, Dict[str, float]] = {}
state_cache_tier: Dict[str, Dict[str, float]] = {}
...
def debug_items():
    payload = {
        "items_columns": list(items_df.columns),
        "items_head": jsonable_encoder(items_df.head(3).to_dict(orient="records")),
    }
    return jsonable_encoder(_sanitize(payload))
...
class QuestionResponse(BaseModel):
    ...
    p_bkt_l2: Optional[float] = None
    p_bkt_tier: Optional[float] = None
    p_irt: Optional[float] = None
    tier: Optional[int] = None
    L1: Optional[str] = None
    L2: Optional[str] = None
    L3: Optional[str] = None
    L2_tier: Optional[str] = None
...
def get_user_state(user_id: str) -> Dict[str, float]:
    ...
    # fallback: logs の p_L_after から推定
    hist = logs_df[logs_df["user_id"].astype(str) == str(user_id)]
    if "p_L_after" in hist.columns and not hist.empty:
        grouped = hist.groupby("domain")["p_L_after"].mean().to_dict()
        return {k: _clean_float(v) for k, v in grouped.items()}
    return {}
...
def get_user_history(user_id: str) -> List[Dict]:
    ...
    # tier/L1/L2/L3 を補完
    meta_cols = [c for c in ("item_id", "tier", "L1", "L2", "L3") if c in items_df.columns]
    if meta_cols:
        hist = hist.merge(items_df[meta_cols], on="item_id", how="left")
        if "L2" in hist.columns and "tier" in hist.columns and "L2_tier" not in hist.columns:
            hist["L2_tier"] = hist["L2"].astype(str) + ":t" + hist["tier"].astype(str)
    # P_final を結合
...
def _get_mood_state(history: List[Dict], window: int = 5) -> str:
    if not history:
        return "NORMAL"
    recent = history[-window:]
    corrects = [int(r.get("correct", 0)) for r in recent if r.get("correct") is not None]
    if not corrects:
        return "NORMAL"
...
def _mood_policy(mood: str) -> Dict[str, float | str]:
    if mood == "FRUSTRATED":
        return {"target": 0.75, "low": 0.60, "high": 0.90, "comment_ja": "成功体験を優先するため解きやすい問題を中心に選ぶ"}
    if mood == "CONFIDENT":
        return {"target": 0.65, "low": 0.50, "high": 0.85, "comment_ja": "挑戦も含めて次のステップに進める問題を選ぶ"}
    return {"target": 0.70, "low": 0.60, "high": 0.85, "comment_ja": "適正難易度を中心にバランスよく選ぶ"}
...
def _build_llm_prompt(
    *,
    ...
    state_l2: Dict[str, float],
    state_tier: Dict[str, float],
) -> Dict:
    mood = _get_mood_state(history)
    policy = _mood_policy(mood)
    recent = history[-5:] if history else []
    tier_mastery = _get_user_tier_mastery(user_id)
    return {
        "user_id": user_id,
        "mood_state": mood,
        "policy_hint": policy,
        ...
        "user_state_l2": state_l2,
        "user_state_tier": state_tier,
...
def select_next_question(
    ...
    if not user_scores.empty:
        ...
        # scores が無い場合は items から候補を作る（P_final=0.5）
        candidates = items_df[~items_df["item_id"].isin(asked)].copy()
        if candidates.empty:
            candidates = items_df.copy()
        candidates["P_final"] = 0.5
    if tier_mode == "prereq":
    ...
    for _, row in candidates.iterrows():
        item_row = items_df[items_df["item_id"] == row["item_id"]]
        question_text = item_row["question_text"].iloc[0] if not item_row.empty else ""
        choices = item_row["choices"].iloc[0] if not item_row.empty else None
        tier_series = item_row.get("tier")
        tier_val = int(tier_series.iloc[0]) if isinstance(tier_series, pd.Series) and not tier_series.empty else None
        l1_series = item_row.get("L1")
        l2_series = item_row.get("L2")
        l3_series = item_row.get("L3")
        l1_val = l1_series.iloc[0] if isinstance(l1_series, pd.Series) and not l1_series.empty else None
        l2_val = l2_series.iloc[0] if isinstance(l2_series, pd.Series) and not l2_series.empty else None
        l3_val = l3_series.iloc[0] if isinstance(l3_series, pd.Series) and not l3_series.empty else None
        l2_tier = f"{l2_val}:t{tier_val}" if l2_val is not None and tier_val is not None else None
        ...
        result.append(
            QuestionResponse(
                ...
                tier=tier_val,
                L1=str(l1_val) if l1_val is not None else None,
                L2=str(l2_val) if l2_val is not None else None,
                L3=str(l3_val) if l3_val is not None else None,
                L2_tier=str(l2_tier) if l2_tier is not None else None,
            )
        )
    return result


def _ensure_state(cache: Dict[str, Dict[str, float]], params_map: Dict[str, Dict[str, float]], user_id: str, skill: str) -> float:
    if not bkt_core or skill not in params_map:
        return 0.5
    user_state = cache.setdefault(user_id, {})
    if skill not in user_state:
        user_state[skill] = params_map[skill]["L0"]
    return user_state[skill]


def _update_state(cache: Dict[str, Dict[str, float]], params_map: Dict[str, Dict[str, float]], user_id: str, skill: str, correct: int) -> Dict[str, float]:
    if not bkt_core or skill not in params_map:
        return {"p_L_after": 0.5, "p_next": 0.5}
    user_state = cache.setdefault(user_id, {})
    params = bkt_core.BKTParams.from_row(
        {"L0": params_map[skill]["L0"], "T": params_map[skill]["T"], "S": params_map[skill]["S"], "G": params_map[skill]["G"]}
    )
    prior = user_state.get(skill, params.L0)
    info = bkt_core.update_state(prior, params, correct)
    user_state[skill] = info["p_L_after"]
    return info


def update_state(user_id: str, domain: str, tier_skill: Optional[str], correct: int) -> Dict[str, float]:
    info_l2 = _update_state(state_cache_l2, bkt_params_l2, user_id, domain, correct)
    info_tier = {"p_L_after": 0.5, "p_next": 0.5}
    if tier_skill:
        info_tier = _update_state(state_cache_tier, bkt_params_tier, user_id, tier_skill, correct)
    p_bkt = (1.0 - W_TIER) * info_l2["p_next"] + W_TIER * info_tier["p_next"]
    return {
        "p_L_after": info_l2["p_L_after"],
        "p_L_after_tier": info_tier["p_L_after"],
        "p_next": p_bkt,
        "p_next_l2": info_l2["p_next"],
        "p_next_tier": info_tier["p_next"],
    }
...
def session_answer(req: AnswerRequest):
    ...
    tier_val = item_row.get("tier", None)
    l2_val = item_row.get("L2", None)
    l2_tier = None
    if l2_val is not None and tier_val is not None:
        l2_tier = f"{l2_val}:t{int(tier_val)}"
    state_info = update_state(req.user_id, domain, l2_tier, is_correct)
...
def session_next(
    ...
    user_state_l2 = state_cache_l2.get(user_id, {})
    user_state_tier = state_cache_tier.get(user_id, {})
    asked = set(logs_df[logs_df["user_id"].astype(str) == str(user_id)]["item_id"].astype(str).tolist())
    ...
    for _, row in pool.iterrows():
        item_id = str(row["item_id"])
        dom = str(row["domain"])
        tier_val = int(row.get("tier", 1)) if not pd.isna(row.get("tier", 1)) else 1
        l1_val = row.get("L1")
        l2_val = row.get("L2")
        l3_val = row.get("L3")
        l2_tier = f"{l2_val}:t{tier_val}" if l2_val is not None else None
        p_bkt_l2 = float(user_state_l2.get(dom, 0.5))
        p_bkt_tier = float(user_state_tier.get(l2_tier, 0.5)) if l2_tier else 0.5
        p_bkt = (1.0 - W_TIER) * p_bkt_l2 + W_TIER * p_bkt_tier
        p_irt = None
        ...
        candidates.append(
            QuestionResponse(
                ...
                choices=choices if isinstance(choices, list) else None,
                p_bkt_l2=_clean_float(p_bkt_l2),
                p_bkt_tier=_clean_float(p_bkt_tier),
                p_irt=_clean_float(p_irt),
                tier=tier_val,
                L1=str(l1_val) if l1_val is not None else None,
                L2=str(l2_val) if l2_val is not None else None,
                L3=str(l3_val) if l3_val is not None else None,
    ...
    if not user_state_l2:
        ...
        if use_llm:
            ...
            prompt = _build_llm_prompt(
                ...
                tier_mode=tier_mode,
                tier_threshold=tier_threshold,
                state_l2=user_state_l2,
                state_tier=user_state_tier,
            )
            llm_choice = _call_openai_llm(prompt, llm_mode, llm_model)
            if llm_choice:
