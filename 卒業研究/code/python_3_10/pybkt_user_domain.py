...
追加機能:
- `--compare-forgets` で **忘却なし vs 忘却あり** の AUC/Logloss 比較（全体/スキル別/ユーザー別/ユーザー×スキル別）
...
def _logloss_safe(y_true: np.ndarray, y_prob: np.ndarray) -> float:
...
def _evaluate_metrics(pred: pd.DataFrame, cols: dict, par: pd.DataFrame, by: str) -> pd.DataFrame:
    score = _ensure_score_series(pred, cols, par)
    y = pred[cols["correct"]].astype(int)
    sk_key = cols.get("skill_name", "skill")
    if by == "overall":
        auc = _roc_auc_safe(y.to_numpy(), score.to_numpy())
        logloss = _logloss_safe(y.to_numpy(), score.to_numpy())
        pos_rate = float(y.mean()) if len(y) else float("nan")
        return pd.DataFrame([{ "scope":"overall", "n": len(y), "pos_rate": pos_rate, "auc": auc, "logloss": logloss }])
    if by == "skill":
        rows = []
        for sk, g in pred.groupby(cols["skill_name"]):
            ...
            rows.append({
                sk_key: str(sk),
                "n": int(g.shape[0]),
                "pos_rate": float(g[cols["correct"]].mean()),
                "auc": _roc_auc_safe(g[cols["correct"]].to_numpy(), s.to_numpy()),
                "logloss": _logloss_safe(g[cols["correct"]].to_numpy(), s.to_numpy())
            })
...
def main():
    ...
    ap.add_argument("--display-skill-label", default="", help="出力上のラベル名（例: 分野）。空なら列名をそのまま使用")
    # === AUC compare options ===
    ...
    if args.compare_forgets:
        ...
        t0 = _evaluate_metrics(pred0, cols, par0 if not par0.empty else par1, by=args.auc_by)
        t0.insert(0, "model", "no_forget")
        t1 = _evaluate_metrics(pred1, cols, par1 if not par1.empty else par0, by=args.auc_by)
        t1.insert(0, "model", "forget")

        # マージして差分
        sk_key = cols.get("skill_name", "skill")
        key_cols = [c for c in ["scope","user", sk_key] if c in t0.columns or c in t1.columns]
        ...
        if "logloss_noF" in m.columns and "logloss_F" in m.columns:
            m["delta_logloss"] = m["logloss_F"] - m["logloss_noF"]
        # 並び替え
    ...
    # pyBKTの state_predictions は観測前（prior）
    last = last.rename(columns={"state_predictions": "P(L)_prior"})
    last[cols["user_id"]] = last[cols["user_id"]].astype(str)
    last[cols["skill_name"]] = last[cols["skill_name"]].astype(str)
    last["P(L)_prior"] = last["P(L)_prior"].astype(float)

    # posterior を観測正誤と S/G から計算（欠損があれば prior を流用）
    if {"p_S","p_G"}.issubset(last.columns) and last[["p_S","p_G"]].notna().any().all():
        y = last[cols["correct"]].astype(int)
        p = last["P(L)_prior"]
        S = last["p_S"].astype(float)
        G = last["p_G"].astype(float)
        post_when_1 = (p*(1-S)) / (p*(1-S) + (1-p)*G + 1e-12)
        post_when_0 = (p*S)     / (p*S     + (1-p)*(1-G) + 1e-12)
        last["P(L)_posterior"] = np.where(y==1, post_when_1, post_when_0)
    else:
        last["P(L)_posterior"] = last["P(L)_prior"]

    # 次時点の prior（学習遷移のみ、忘却なし）
    if "p_T" in last.columns and last["p_T"].notna().any():
    ...
    out_cols = [cols["user_id"], cols["skill_name"], "n_obs",
                "P(L)_prior", "P(L)_posterior", "P(L)_after_learn", "P(next correct)",
                "p_L0", "p_T", "p_S", "p_G"]
    ...
    # 表示ラベルの差し替え（任意）
    if args.display_skill_label:
        if cols["skill_name"] in out.columns:
            out = out.rename(columns={cols["skill_name"]: args.display_skill_label})
    ...
    if args.plot_out:
        ...
        label_nm = args.display_skill_label if args.display_skill_label else ("domain" if cols["skill_name"] != "skill" else "skill")
        _plot_user_skills_timeseries(pred, par, args.user, skills_to_plot, cols, args.plot_out, label_nm)
        print(f"plot saved -> {args.plot_out}")


def _plot_user_skills_timeseries(pred: pd.DataFrame, par: pd.DataFrame, user, skills, cols, out_path, label_name="skill"):
    try:
    ...
    for i, sk in enumerate(skills):
        ...
        ax.set_title(f"user={user_str}  {label_name}={sk}  n={n_obs}")
        if i == 0:
