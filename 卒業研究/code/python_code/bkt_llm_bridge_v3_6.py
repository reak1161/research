...
# -*- coding: utf-8 -*-
"""
bkt_llm_bridge_v3_6.py

追加:
- --holdout_ratio r       : 各 user×skill で末尾 r(0<r<1) 割合を評価用に分離（時間順）
- --holdout_min_eval k    : 評価区間の最小件数（既定1）
- 優先順位: cut_date > holdout_last_k > holdout_ratio
- fit_exclude_holdout, report_scope など v3.5 の機能はそのまま

注意:
- AUC/Logloss は「分割後の評価区間」で算出（さらに --metrics_window で末尾N件）
- ランキングや P(next) は report_scope（full/train）に従う
"""
...
def bkt_update_pL_with_obs(pL: float, p: Dict[str,float], obs: int) -> float:
    pL_prior = pL + (1 - pL) * p["p_T"]
    num = ((1 - p["p_S"]) if obs==1 else p["p_S"]) * pL_prior
    den = num + ((p["p_G"]) if obs==1 else (1 - p["p_G"])) * (1 - pL_prior)
    return pL_prior if den<=0 else num/den

def bkt_predict_next_from_obs(obs_list: List[int], p: Dict[str,float]) -> Tuple[float,float,float]:
    pL = p["p_L0"]
    for o in obs_list:
        pL = bkt_update_pL_with_obs(pL, p, o)
...
# ===== 時系列分割 =====
def split_train_eval_obs_for_user_skill(
    sdf: pd.DataFrame,
    holdout_last_k:int=0,
    cut_date:Optional[pd.Timestamp]=None,
    holdout_ratio:Optional[float]=None,
    holdout_min_eval:int=1
) -> Tuple[List[int], List[int]]:
    """ユーザー×スキル（date昇順）の df から、学習用と評価用の obs を返す"""
    # 1) cut_date 優先
    if cut_date is not None:
        train = sdf[sdf["date"] <= cut_date]["result"].astype(int).tolist()
        eval_ = sdf[sdf["date"] >  cut_date]["result"].astype(int).tolist()
        return train, eval_
    ...
    if n == 0:
        return [], []
    # 2) K優先
    if holdout_last_k and holdout_last_k > 0:
        k = min(holdout_last_k, n)
        return ys[:-k] if k < n else [], ys[-k:]
    # 3) 割合
    if holdout_ratio is not None:
        r = float(holdout_ratio)
        if not (0.0 < r < 1.0):
            raise ValueError("--holdout_ratio は 0と1の間で指定してください（例: 0.2）")
        k = int(np.floor(n * r))
        k = max(holdout_min_eval, k)
        if k >= n:
            # 評価が全量にならないよう1件は学習に残す
            k = n-1
        return ys[:-k], ys[-k:]
    # 4) 指定なし
    return ys, []

def make_fit_dataframe(
    df: pd.DataFrame,
    holdout_last_k:int,
    cut_date:Optional[str],
    fit_exclude_holdout:bool,
    holdout_ratio:Optional[float],
    holdout_min_eval:int
) -> pd.DataFrame:
    """EM学習用 df を返す。fit_exclude_holdout=True なら評価区間を除外"""
    if not fit_exclude_holdout:
        return df
    if cut_date:
    ...
    if holdout_last_k and holdout_last_k > 0:
        d = df.copy()
        d["grp_size"] = d.groupby(["user_id","skill_id"])["result"].transform("size")
        d["rank"]     = d.groupby(["user_id","skill_id"]).cumcount()
        out = d[d["rank"] < (d["grp_size"] - holdout_last_k)].drop(columns=["grp_size","rank"])
        return out
    if holdout_ratio is not None:
        r = float(holdout_ratio)
        if not (0.0 < r < 1.0):
            raise ValueError("--holdout_ratio は 0と1の間で指定してください（例: 0.2）")
        d = df.copy()
        g = d.groupby(["user_id","skill_id"], sort=False)
        d["grp_size"] = g["result"].transform("size")
        d["rank"]     = g.cumcount()
        # 評価件数 k = max(min_eval, floor(n*r))、ただし k < n
        k = (np.floor(d["grp_size"] * r)).astype(int)
        k = np.maximum(k, holdout_min_eval)
        k = np.where(k >= d["grp_size"], d["grp_size"]-1, k)
        # 学習側: rank < (grp_size - k)
        mask_train = d["rank"] < (d["grp_size"] - k)
        out = d[mask_train].drop(columns=["grp_size","rank"])
        return out
    return df
...
# ===== Scoring =====
def parse_tags_safe(val: Optional[str]) -> List[str]:
    if pd.isna(val): return []
    s=str(val).strip()
    return [t.strip() for t in s.split(",") if t.strip()] if s else []

def score_teach(pnext,n_obs,pT,pL,item_row,goal_skills:set,goal_tags:set,sweet=0.65):
    sid=int(item_row["skill_id"]); tags=set(parse_tags_safe(item_row.get("kc_tags","")))
    coverage=0.0
...
# ===== Metrics: EIG/entropy, AUC/Logloss =====
def _H2(p):
...
def forward_preds_on_eval(train_obs: List[int], eval_obs: List[int], p: Dict[str,float]) -> Tuple[List[int], List[float]]:
    pL = p["p_L0"]
    for o in train_obs:
        pL = bkt_update_pL_with_obs(pL, p, o)
    y_true=[]; y_pred=[]
    for o in eval_obs:
        ...
        y_true.append(int(o)); y_pred.append(float(phat))
        pL = bkt_update_pL_with_obs(pL, p, o)
...
def compute_user_skill_metrics_split(
    df_hist: pd.DataFrame,
    params_by_skill: Dict[int,Dict[str,float]],
    user_id:int, skill_id:int,
    holdout_last_k:int=0, cut_date:Optional[str]=None,
    holdout_ratio:Optional[float]=None, holdout_min_eval:int=1,
    window:int=0
) -> Dict[str,float]:
    ...
    cut_ts = pd.to_datetime(cut_date) if cut_date else None
    train_obs, eval_obs = split_train_eval_obs_for_user_skill(
        sdf, holdout_last_k, cut_ts, holdout_ratio, holdout_min_eval
    )
    if not eval_obs:
        # 評価区間が空 → 従来方式（全区間）にフォールバック
        ys = sdf["result"].astype(int).tolist()
        if window and len(ys)>window:
            ys = ys[-window:]
        pL = p["p_L0"]; y_true=[]; y_pred=[]
        for o in ys:
            pL_prior = pL + (1 - pL) * p["p_T"]
            phat = (1 - p["p_S"]) * pL_prior + p["p_G"] * (1 - pL_prior)
            y_true.append(int(o)); y_pred.append(float(phat))
    ...
    if window and len(eval_obs)>window:
        eval_obs = eval_obs[-window:]
    y_true, y_pred = forward_preds_on_eval(train_obs, eval_obs, p)
    return {"auc": roc_auc(y_true, y_pred), "logloss": binary_logloss(y_true, y_pred)}
    ...
    for r in ranked_df.itertuples(index=False):
        ...
        if include_metrics_in_payload:
            ...
            for key in ["eig_bits","p_slip_event","p_guess_event","H_prior_bits","auc","logloss"]:
                if hasattr(r, key):
                    val = getattr(r, key)
                    metrics[key] = None if (val is None or (isinstance(val, float) and np.isnan(val))) else round(float(val), 3)
            if metrics:
...
def main():
    ...
    ap.add_argument("--metrics_window", type=int, default=0, help="AUC/Loglossの直近件数（0=評価区間の全件）")
    # 時系列分割
    ap.add_argument("--holdout_last_k", type=int, default=0, help="各 user×skill の末尾K件を評価に分離")
    ap.add_argument("--cut_date", default=None, help="YYYY-MM-DD。これより後を評価に分離")
    ap.add_argument("--fit_exclude_holdout", action="store_true", help="EM学習から評価区間を除外")
    ap.add_argument("--report_scope", choices=["full","train"], default="full", help="P(next)等を全履歴/学習側で計算")
    # NEW: 割合ホールドアウト
    ap.add_argument("--holdout_ratio", type=float, default=None, help="(0,1) 各 user×skill 末尾の評価割合 例: 0.2")
    ap.add_argument("--holdout_min_eval", type=int, default=1, help="割合分割時の評価区間の最小件数")
    # 出力
    ...
    # 3) EM学習データ（評価区間の除外オプション対応）
    df_fit = make_fit_dataframe(
        df,
        holdout_last_k=args.holdout_last_k,
        cut_date=args.cut_date,
        fit_exclude_holdout=args.fit_exclude_holdout,
        holdout_ratio=args.holdout_ratio,
        holdout_min_eval=args.holdout_min_eval
    )

    # 4) 推定
    params=fit_by_skill(df_fit, skills)

    # 5) レポート（report_scope）
    report_rows=[]
    cut_ts = pd.to_datetime(args.cut_date) if args.cut_date else None
    for sid in skills:
        sdf_user = df[(df["user_id"]==args.user_id) & (df["skill_id"]==sid)].sort_values("date")
        if args.report_scope=="train" and (args.holdout_last_k>0 or cut_ts is not None or args.holdout_ratio is not None):
            train_obs, _ = split_train_eval_obs_for_user_skill(
                sdf_user,
                holdout_last_k=args.holdout_last_k,
                cut_date=cut_ts,
                holdout_ratio=args.holdout_ratio,
            ...
            obs_for_report = train_obs
        else:
            obs_for_report = sdf_user["result"].astype(int).tolist()
        p = params.get(int(sid)) or {"p_L0":0.2,"p_T":0.1,"p_S":0.1,"p_G":0.2,"loglik":float("nan"),"iters":0}
        pL, pL_next, p_corr = bkt_predict_next_from_obs(obs_for_report, p)
        report_rows.append({"user_id":int(args.user_id), "skill_id":int(sid), "n_obs":len(obs_for_report),
                            "P(L)_posterior":pL, "P(L)_after_learn":pL_next,
                            "P(next correct)":p_corr, **p})
    report_df = pd.DataFrame(report_rows)
    if args.print_report:
        print("# BKT Report (user×skills)")
        print(report_df.to_string(index=False)); print()
    ...
    if need_user_metrics:
        for sid in skills:
            skill_metrics[int(sid)] = compute_user_skill_metrics_split(
                df, params, args.user_id, int(sid),
                holdout_last_k=args.holdout_last_k,
                cut_date=args.cut_date,
                holdout_ratio=args.holdout_ratio,
                holdout_min_eval=args.holdout_min_eval,
