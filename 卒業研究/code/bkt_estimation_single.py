...
# ===== 1) データ読み込み（単一スキル前提） =====
# 先頭5万行だけ読み込む（ファイル順）
#df = pd.read_csv("../csv/one_200_50_500_50000.csv", nrows=50000)
df = pd.read_csv("../csv/one_200_50_500_50000.csv")
df["date"] = pd.to_datetime(df["date"])

# 既に各行が単一スキルを表している前提で、そのまま使用
df_single = df.copy()
...
# 例：ユーザ68, 42, 1のレポート（元スクリプトの出力形式に合わせて表示）
