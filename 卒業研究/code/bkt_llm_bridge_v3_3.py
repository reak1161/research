...
# -*- coding: utf-8 -*-
"""
bkt_llm_bridge_v3_3.py

追加点:
- --eval_indices で評価指数を選択表示（eig, slip, guess, entropy）
- --show_indices でコンソール表示に列追加
- --include_metrics_in_payload で LLMへ渡す candidates[] に metrics を同梱
  （LLMの出力スキーマは不変更）
"""
...
def expected_info_gain_bits(p_prior, p_S, p_G):
    """観測前の P(L)=p_prior と S,G から、1問出したときの期待情報利得(bit)を計算"""
    p = float(np.clip(p_prior, 1e-12, 1-1e-12))
    S = float(np.clip(p_S,    1e-9,  1-1e-9))
    G = float(np.clip(p_G,    1e-9,  1-1e-9))
    # 事前（観測直前）
    H_prior = _H2(p)
    # 観測の正答確率
    pC = (1 - S) * p + G * (1 - p)
    pI = 1 - pC
    # 事後（正答/誤答）それぞれ
    pL_C = ((1 - S) * p) / float(np.clip(pC, 1e-12, 1))
    pL_I = (S * p) / float(np.clip(S*p + (1 - G)*(1 - p), 1e-12, 1))
    H_post = pC * _H2(pL_C) + pI * _H2(pL_I)
    return H_prior - H_post

def compute_metrics_row(r_skill, p_for_llm, center):
    """rank_candidates 内で作る行に、必要なら評価指数を計算して返す"""
    # r_skill: report_df.loc[sid]（Series）
    p_prior = float(r_skill["P(L)_after_learn"])
    pS = float(r_skill["p_S"]); pG = float(r_skill["p_G"])
    return {
        "eig_bits": expected_info_gain_bits(p_prior, pS, pG),
        "p_slip_event": pS * p_prior,
        "p_guess_event": pG * (1 - p_prior),
        "H_prior_bits": _H2(p_prior),
    }
...
def rank_candidates(report_df, items_df, user_id, mode="motivate",
    ...
    """eval_indices に指定された評価指数を列として付与（ランキングは従来通り score のみ）"""
    ...
    for _,it in items_df.iterrows():
        ...
        # 追加: 評価指数
        metrics = compute_metrics_row(r, p_for_llm, center_for_dist)
        if "eig"      in eval_indices: row["eig_bits"]     = metrics["eig_bits"]
        if "slip"     in eval_indices: row["p_slip_event"] = metrics["p_slip_event"]
        if "guess"    in eval_indices: row["p_guess_event"]= metrics["p_guess_event"]
        if "entropy"  in eval_indices: row["H_prior_bits"] = metrics["H_prior_bits"]
        rows.append(row)
    cand=pd.DataFrame(rows)
    if cand.empty: return cand
    return cand.sort_values("score", ascending=False).head(topn)

# ===== Mix & diversity（v3.1修正版）=====
...
def build_learner_profile(df_history: pd.DataFrame, report_df: pd.DataFrame, user_id: int, level: str, candidate_skills: List[int]):
    ...
    # full のときだけ per-skill 数値を同梱（列名アクセスで安全に）
    if level == "full":
        per = {}
...
def build_llm_messages(user_id, ranked_df, mode="motivate", goals=None, language="ja",
    ...
    for r in ranked_df.itertuples(index=False):
        ...
        if include_metrics_in_payload:
            metrics={}
            for key in ["eig_bits","p_slip_event","p_guess_event","H_prior_bits"]:
                if hasattr(r, key):
                    metrics[key] = round(float(getattr(r,key)), 3)
            if metrics:
...
def main():
    ...
    # 評価指数（NEW）
    ap.add_argument("--eval_indices", default="", help="カンマ区切りで eig, slip, guess, entropy を指定")
    ap.add_argument("--show_indices", action="store_true", help="コンソール表示に評価指数の列を含める")
    ap.add_argument("--include_metrics_in_payload", action="store_true", help="candidates[].metrics に評価指数を同梱")
    ap.add_argument("--round_indices", type=int, default=3, help="表示・ペイロード用の小数桁（評価指数）")
    # 出力
    ...
    if args.show_indices:
        cols_round = [c for c in ["eig_bits","p_slip_event","p_guess_event","H_prior_bits"] if c in ranked.columns]
        if cols_round:
