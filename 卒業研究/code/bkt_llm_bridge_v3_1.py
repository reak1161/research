...
# -*- coding: utf-8 -*-
"""
bkt_llm_bridge_v3_1.py  (2025-10-24)

差分（v3→v3.1）:
- 配分保証(enforce_mix)の重複バグを修正（同一(item_id,skill_id)が選ばれない）
- ランキング直後に row_id を付与してユニーク管理
- 空DataFrameのconcatを避けて FutureWarning を抑制
- skill多様化（--prefer_unique_skills）をより確実に

機能概要:
- BKTで user×skill の学習度・次回正答確率を推定
- teach / motivate / hybrid の目的別スコア
- 難易度タグ（easy/med/hard）で per-item 期待正答率補正
- enforce_mix で practice/review/challenge の配分＆スキル多様化を保証
- LLMに渡す System/User メッセージ＆スキーマヒントを生成
- （任意）モック応答JSONも出力
"""
...
def predict_user_skill(df_single: pd.DataFrame, params_by_skill: Dict[int,Dict[str,float]], user_id: int, skill_id: int) -> pd.DataFrame:
    ...
    return pd.DataFrame([{"user_id":int(user_id), "skill_id":int(skill_id), "n_obs":len(obs), "P(L)_posterior":pL, "P(L)_after_learn":pL_next, "P(next correct)":p_corr, **p}])
...
# ========= スコア =========
def score_teach(pnext, n_obs, pT, pL_post, item_row, goal_skills:set, goal_tags:set, sweet=0.65):
    ...
    deficit = (1.0 - pL_post); learn_gain = pT * (1.0 - pL_post)
    sweet_term = -abs(pnext - sweet); explore = 0.10/np.sqrt(n_obs+1.0)
...
def parse_tag_deltas(tag_deltas_str: str):
    vals = [float(x) for x in str(tag_deltas_str).split(",")]
    if len(vals) != 3: raise ValueError("--tag_deltas は 'easy,med,hard' の3要素で指定してください（例: +0.12,0,-0.12）")
...
def enforce_mix(ranked_df: pd.DataFrame, mix_str="3,1,1",
    ...
    """ 重複排除・スキル多様化を考慮して practice, review, challenge の配分を満たす """
    ...
    def pick(block: pd.DataFrame, k: int, prefer_unique: bool):
        picked = []
        if k<=0 or block.empty: return picked
        for _, row in block.iterrows():
            ...
            picked.append(row); used_rowids.add(rid); used_keys.add(key); used_skills.add(sid)
...
# ========= LLMメッセージ =========
def build_llm_messages(user_id: int, ranked_df: pd.DataFrame, mode="motivate", goals=None, language="ja"):
...
def main():
    ...
    ap.add_argument("--items_csv", default=None, help="候補アイテムCSV: item_id, skill_id[, difficulty_tag, kc_tags, engagement, est_sec, ...]")
    ...
    ap.add_argument("--tag_deltas", default="+0.12,0,-0.12", help="easy,med,hard の補正（例: +0.12,0,-0.12）")
    # 配分・多様性
    ap.add_argument("--mix", default="3,1,1", help="practice,review,challenge の配分（合計=5目安）")
    ...
    ap.add_argument("--prefer_unique_skills", action="store_true", help="可能な範囲で skill_id の重複を避ける")
    # 出力
    ap.add_argument("--topn", type=int, default=12, help="配分前のランキング上位を何件プールするか")
    ...
    system_msg, user_msg, schema_hint, payload = build_llm_messages(args.user_id, ranked, mode=args.mode, goals=goals_hint)
