...
# -*- coding: utf-8 -*-
"""
bkt_llm_bridge_v3_5.py

- 時系列分割を追加:
  --holdout_last_k K         : 各 user×skill の末尾 K 件を評価用に分離
  --cut_date YYYY-MM-DD      : 日付で学習/評価を分割（cut_date より後が評価）
  --fit_exclude_holdout      : 学習(EM)から評価区間を除外して推定
  --report_scope full|train  : レポート/ランキングを全履歴 or 学習側のみで計算

- AUC/Logloss は分割後の「評価区間」で算出（さらに --metrics_window で末尾N件に絞り込み可）
- 既存の EIG/entropy なども従来通り利用可能
"""
...
def bkt_predict_next_from_obs(obs_list: List[int], p: Dict[str,float]) -> Tuple[float,float,float]:
    """既存: 全観測を通した後の P(L), 次時点 P(L), 次回正答率"""
    pL = p["p_L0"]
    for o in obs_list:
...
# ===== 時系列分割 =====
def split_train_eval_obs_for_user_skill(sdf: pd.DataFrame, holdout_last_k:int=0, cut_date:Optional[pd.Timestamp]=None) -> Tuple[List[int], List[int]]:
    """ユーザー×スキルの df（date昇順）から、学習用と評価用の obs リストを返す"""
    ...
    if holdout_last_k and holdout_last_k > 0 and len(ys) > 0:
        k = min(holdout_last_k, len(ys))
        return ys[:-k], ys[-k:]
    return ys, []  # 分割なし

def make_fit_dataframe(df: pd.DataFrame, holdout_last_k:int, cut_date:Optional[str], fit_exclude_holdout:bool) -> pd.DataFrame:
    """EM学習用の df を返す。fit_exclude_holdout=True なら評価区間を除外"""
    ...
    if holdout_last_k and holdout_last_k > 0:
        ...
        return d[d["rank"] < (d["grp_size"] - holdout_last_k)].drop(columns=["grp_size","rank"])
    return df
...
def forward_preds_on_eval(train_obs: List[int], eval_obs: List[int], p: Dict[str,float]) -> Tuple[List[int], List[float]]:
    """学習区間で pL を進めてから、評価区間で逐次予測確率を出す"""
    pL = p["p_L0"]
    for o in train_obs:
        pL = bkt_update_pL_with_obs(pL, p, o)
    y_true=[]; y_pred=[]
...
def compute_user_skill_metrics_split(df_hist: pd.DataFrame, params_by_skill: Dict[int,Dict[str,float]],
    ...
    train_obs, eval_obs = split_train_eval_obs_for_user_skill(sdf, holdout_last_k, cut_ts)
    if not eval_obs:
        # 分割指定なし or 評価区間が空 → 従来方式（全区間で逐次予測）にフォールバック
        ys = sdf["result"].astype(int).tolist()
...
def main():
    ...
    ap.add_argument("--metrics_window", type=int, default=0, help="AUC/Loglossの計算に使う直近件数（0=評価区間の全件）")
    # 時系列分割（NEW）
    ap.add_argument("--holdout_last_k", type=int, default=0, help="各 user×skill の末尾K件を評価用に分離")
    ap.add_argument("--cut_date", default=None, help="YYYY-MM-DD。これより後を評価に分離")
    ap.add_argument("--fit_exclude_holdout", action="store_true", help="EM学習から評価区間を除外して推定")
    ap.add_argument("--report_scope", choices=["full","train"], default="full", help="P(next)やレポートを全履歴/学習側で計算")
    # 出力
    ...
    # 3) EM学習用データ（分割の学習側）
    df_fit = make_fit_dataframe(df, args.holdout_last_k, args.cut_date, args.fit_exclude_holdout)

    # 4) BKT推定
    params=fit_by_skill(df_fit, skills)

    # 5) レポート用観測（report_scope）
    report_rows=[]
    ...
    for sid in skills:
        ...
        if args.report_scope=="train" and (args.holdout_last_k>0 or cut_ts is not None):
            train_obs, _ = split_train_eval_obs_for_user_skill(sdf_user, args.holdout_last_k, cut_ts)
    ...
    # 8) ユーザー×スキルの AUC/Logloss を分割評価で計算
    ...
    if need_user_metrics:
        for sid in skills:
            skill_metrics[int(sid)] = compute_user_skill_metrics_split(
                ...
                holdout_last_k=args.holdout_last_k, cut_date=args.cut_date,
