...
【本バージョンの要点】
- explode後の重みを 1/m ではなく **1/√m** に変更（m=観測に紐づくスキル数）
  → 多スキル観測を“薄め過ぎない”中庸設定。EMの有効サンプル数が確保されやすく、
     複数スキルの AND 潰れが強いデータでも推定が安定しやすい。
"""
...
def prepare_exploded(df_all: pd.DataFrame) -> pd.DataFrame:
    """
    skill='a,b,c' を行分解し、重み = **1/√m** を付与（m=その観測に含まれるスキル数）
    - 例: m=1 → 1, m=2 → 1/√2, m=4 → 1/2
    """
    # スキル分解（空白想定なしの元実装に合わせる）
    skl = df_all["skill"].astype(str).str.split(",")
    m = skl.str.len().astype(float).clip(lower=1.0)
    ...
    # ★ ここが 1/m → 1/√m への変更点
    ex["weight"] = 1.0 / np.sqrt(ex["m"].astype(float).clip(lower=1.0))
...
if __name__ == "__main__":
    ...
    if USE_WEIGHTED_EM:
        ...
        print("Top-10 skills by effective samples (√-weighted):")
